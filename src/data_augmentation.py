...
def colorize_two(grayimg1, grayimg2, maxcolornum):
    ...
    checkpoints = []
    checkpoints += [int(np.round(i*(maxdist/(colornum-1.0))))
    ...
    colorimg1 = np.clip(cv2.GaussianBlur(
        colorimg1, (3, 3), 3, 1), a_min=0, a_max=255) / 255.0
    ...
    colorimg2 = np.clip(cv2.GaussianBlur(
        colorimg2, (3, 3), 3, 1), a_min=0, a_max=255) / 255.0
