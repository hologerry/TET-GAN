...
from torch.nn import Conv2d, BatchNorm2d, LeakyReLU, ConvTranspose2d, ReLU, Tanh, InstanceNorm2d
...
class Discriminator(nn.Module):
    def __init__(self, in_channels, ndf=64, n_layers=3):
        ...
        for n in range(1, n_layers):
            ...
            modelList.append(InstanceNorm2d(
                num_features=ndf * nf_mult, affine=True))
            modelList.append(LeakyReLU(0.2))
...
class Encoder(nn.Module):
    def __init__(self, nef=32, useIn=False):
        ...
        self.rcb1 = RCBBlock(
            in_channels=nef*2, out_channels=nef*4, useIn=useIn)
        self.rcb2 = RCBBlock(
            in_channels=nef*4, out_channels=nef*8, useIn=useIn)
        self.rcb3 = RCBBlock(
            in_channels=nef*8, out_channels=nef*16, useIn=useIn)
        self.rcb4 = RCBBlock(in_channels=nef*16,
                             out_channels=nef*16, useIn=useIn)
...
class TETGAN(nn.Module):
    def __init__(self, nef=32, ndf=64, gpu=True):
        ...
        self.trainerE_x = torch.optim.Adam(
            self.E_x.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.trainerE_yc = torch.optim.Adam(
            self.E_yc.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.trainerS_x = torch.optim.Adam(
            self.S_x.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.trainerG_x = torch.optim.Adam(
            self.G_x.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.trainerE_ys = torch.optim.Adam(
            self.E_ys.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.trainerS_y = torch.optim.Adam(
            self.S_y.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.trainerG_y = torch.optim.Adam(
            self.G_y.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.trainerD_x = torch.optim.Adam(
            self.D_x.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.trainerD_y = torch.optim.Adam(
            self.D_y.parameters(), lr=0.0002, betas=(0.5, 0.999))
    ...
    def update_destylization_discriminator(self, y, y2, x, level, w):
        ...
        gradient_penalty = self.calc_gradient_penalty(
            self.D_x, real_concat.data, fake_concat.data)
        Ldadv = fake_output.mean() - real_output.mean() + \
    ...
    def update_stylization_discriminator(self, x, x2, y, y2, y_real, level, w):
        ...
        gradient_penalty = self.calc_gradient_penalty(
            self.D_y, real_concat.data, fake_concat.data)
        Lsadv = fake_output.mean() - real_output.mean() + \
            self.lambda_gp * gradient_penalty
    ...
    def one_pass(self, x, x2, y, y2, y_real, y2_real, level, w):
        ...
        Ldadv = self.update_destylization_discriminator(
            y_real, y2_real, x, level, w)
        Ldesty = self.update_destylization_generator(
            y_real, y2_real, x, x2, level, w)
        Lsadv = self.update_stylization_discriminator(
            x, x2, y, y2, y_real, level, w)
        Lsty = self.update_stylization_generator(
            x, x2, y, y2, y_real, level, w)
