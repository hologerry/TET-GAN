...
def prepare_batch(batchfnames, level=1, jitter=0.0, centercropratio=0.5, augementratio=0.0, gpu=True, wd=64, ht=64):
    ...
    for fname1, fname2 in batchfnames:
        ...
        for i in range(layernum):
            img1_in_ = transform(img1_in.resize(
                (img_wds[i], img_hts[i]), Image.LANCZOS))
            img2_in_ = transform(img2_in.resize(
                (img_wds[i], img_hts[i]), Image.LANCZOS))
            img1_out_ = transform(img1_out.resize(
                (img_wds[i], img_hts[i]), Image.LANCZOS))
            img2_out_ = transform(img2_out.resize(
                (img_wds[i], img_hts[i]), Image.LANCZOS))
